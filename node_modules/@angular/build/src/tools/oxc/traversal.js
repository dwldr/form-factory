 ...
 */
const executableTypeScriptNodes = new Set([
    'TSEnumDeclaration',
    'TSEnumBody',
    'TSEnumMember',
    'TSModuleDeclaration',
 ...
 */
function isTypeOnlyTypeScriptNode(type) {
 ...
 */
function pushNodesReverse(nodes, stack) {
    if (nodes) {
...
function pushChildNodes(node, stack) {
    switch (node.type) {
        ...
        default: {
            ...
            const keys = oxc_parser_1.visitorKeys[node.type];
            if (!keys) {
                break;
            }
 ...
 */
function traversePostOrder(root, visit) {
    const stack = [root];
    let functionDepth = 0;
    let classDepth = 0;
    const functionStack = [];
