...
const BLOCKING_SYMBOL = /* @__PURE__ */Symbol(typeof ngDevMode === 'undefined' || ngDevMode ? '__isBlocking' : '');
const SOURCE_RESOURCE_SYMBOL = /* @__PURE__ */Symbol(typeof ngDevMode === 'undefined' || ngDevMode ? '__sourceResource' : '');
...
function processRoute(route, newlyCreatedRoutes, resourceSetupPromises, abortSignal, blockingResourcePromises) {
  ...
  if (newlyCreatedRoutes.has(route)) {
    resourceSetupPromises.push(setupNewRouterResources(route, resources, abortSignal, blockingResourcePromises));
  } else {
...
async function setupNewRouterResources(route, resourcesFn, abortSignal, blockingResourcePromises) {
  ...
  runInInjectionContext(childInjector, () => {
    for (const [key, res] of Object.entries(resourceResult)) {
      if ((typeof ngDevMode === 'undefined' || ngDevMode) && (!res || typeof res !== 'object' || typeof res.snapshot !== 'function')) {
...
function withRouterResources() {
  ...
  return routerFeature(12, providers);
}
...
export { NoPreloading, PreloadAllModules, PreloadingStrategy, ROUTER_INITIALIZER, ROUTER_PROVIDERS, RouterLink, RouterLinkActive, RouterModule, RouterPreloader, nonBlocking, provideRouter, withAutoCleanupInjectors, withComponentInputBinding, withDebugTracing, withDisabledInitialNavigation, withEnabledBlockingInitialNavigation, withExperimentalAutoCleanupInjectors, withExperimentalPlatformNavigation, withHashLocation, withInMemoryScrolling, withNavigationErrorHandler, withPreloading, withRouterConfig, withRouterResources, withViewTransitions };
